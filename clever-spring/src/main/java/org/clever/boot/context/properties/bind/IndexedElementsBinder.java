...
import org.clever.boot.context.properties.source.IterableConfigurationPropertySource;
...
/**
 * 用于读取索引项的连续运行的{@link AggregateBinder AggregateBinders}的基类。
 * <p>
 * 作者：lizw <br/>
...
abstract class IndexedElementsBinder<T> extends AggregateBinder<T> {
     ...
     * 将索引元素绑定到提供的集合。
     *
     * @param name          要绑定的属性的名称
     * @param target        目标可绑定
     * @param elementBinder 用于元件的粘合剂
     * @param aggregateType 聚合类型可以是集合或数组
    ...
    private void bindValue(Bindable<?> target,
    ...
    /**
     * 索引集合的 {@link AggregateBinder.AggregateSupplier AggregateSupplier}
     */

...
/**
 * ConcurrentHashMap对key和value使用"软引用"或"弱引用"<br/>
 * 此类可以用{@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())}的替代方案，以便在并发访问时支持更好的性能<br/>
 * 此实现遵循与ConcurrentHashMap相同的设计约束，但支持null value和null key<br/>
 * <p>
 * 作者：lizw <br/>
 * 创建时间：2022/04/19 09:55 <br/>
 */
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    private abstract class Task<T> {
        ...
        /**
         * Convenience method that can be used for tasks that do not need to access to {@link Entries}.
         *
         * @param ref   the found reference (or {@code null})
         * @param entry the found entry (or {@code null})
         * @return the result of the task

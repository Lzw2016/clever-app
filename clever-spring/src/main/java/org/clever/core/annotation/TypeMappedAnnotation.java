...
/**
 * {@link MergedAnnotation}的实现，基于{@link AnnotationTypeMapping}实现
 * <p>
 * 作者：lizw <br/>
 * 创建时间：2022/04/22 13:31 <br/>
 */
    ...
    private Object adaptForAttribute(Method attribute, Object value) {
        ...
        if (attributeType.isArray() && attributeType.getComponentType().isAnnotation()) {
            ...
            for (int i = 0; i < result.length; i++) {
                result[i] = adaptToMergedAnnotation(Array.get(value, i), (Class<? extends Annotation>) attributeType.getComponentType());
            }
            return result;
        }
        ...
        if (!attributeType.isInstance(value)) {
            throw new IllegalStateException(
                    "Attribute '" + attribute.getName() + "' in annotation " + getType().getName() +

...
/**
 * 在{@link AnnotatedElement}的注解层级中查找对应Annotation的注解扫描器
 * <p>
 * 作者：lizw <br/>
 * 创建时间：2022/04/22 13:17 <br/>
 */
abstract class AnnotationsScanner {
    ...
    /**
     * 扫描指定元素的层次结构以查找相关注解，并根据需要调用处理器
     *
     * @param context        将传回处理器的可选上下文对象
     * @param source         要扫描的源element
    ...
    private static <C, R> R process(C context, AnnotatedElement source, SearchStrategy searchStrategy, AnnotationsProcessor<C, R> processor) {
    ...
    private static <C, R> R processClassHierarchy(C context,
                                                  int[] aggregateIndex,
                                                  Class<?> source,
                                                  AnnotationsProcessor<C, R> processor,
                                                  boolean includeInterfaces,
    ...
    private static boolean isWithoutHierarchy(AnnotatedElement source, SearchStrategy searchStrategy) {
        ...
        if (source instanceof Class) {
            ...
            return (searchStrategy == SearchStrategy.TYPE_HIERARCHY_AND_ENCLOSING_CLASSES ? noSuperTypes && sourceClass.getEnclosingClass() == null : noSuperTypes);
        }
        if (source instanceof Method) {

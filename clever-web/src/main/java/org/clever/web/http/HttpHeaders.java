...
//     * Object or a {@link org.clever.core.io.Resource} for each part,
...
//     *
//     * @param headerName    the header name
//     * @param rejectInvalid whether to reject invalid values with an {@link IllegalArgumentException} ({@code true}) or rather return -1 in that case ({@code false})
...
//     *
//     * @param headerName    the header name
//     * @param rejectInvalid whether to reject invalid values with an {@link IllegalArgumentException} ({@code true}) or rather return {@code null} in that case ({@code false})
...
//     * @return a read-only variant of the headers, or the original headers as-is (in case it happens to be a read-only {@code HttpHeaders} instance already)
//     */
...
//            return entry.getKey() + ":" + (values.size() == 1 ? "\"" + values.get(0) + "\"" : values.stream().map(s -> "\"" + s + "\"").collect(Collectors.joining(", ")));

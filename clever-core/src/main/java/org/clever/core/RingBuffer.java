...
public class RingBuffer<T> implements Serializable {
     ...
     * 缓冲区第一个元素的索引位置(从0开始)
     */
    private final AtomicLong firstIndex = new AtomicLong(-1);
    /**
     * 缓冲区最后一个元素的索引位置(从0开始)
     */
    ...
    public boolean add(T item) {
        ...
        Boolean res = exclusiveOperation(param -> {
            ...
            if (firstIndex.get() < 0 || last >= bufferSize) {
    ...
    public BufferContent<T> getBuffer() {
        return exclusiveOperation(param -> {
            ...
            if (first < 0 || last < 0) {
                return new BufferContent<>(0, 0, Collections.emptyList());
            }
            ...
            return new BufferContent<>(first + 1, last + 1, logList);
        });
    }
     ...
     * 获取数据
     *
     * @param startIndex 第一条数据的位置(从0开始)
     * @param size       最大获取的数据量
     */
    public BufferContent<T> getBuffer(final long startIndex, long size) {
        ...
        return exclusiveOperation(param -> {
            ...
            if (end >= start) {
                ...
                for (int index = start; index <= end; index++) {
                    int idx = index >= bufferSize ? index % bufferSize : index;
                    logList.add(buffer[idx]);
                }
            }
            return new BufferContent<>(first + 1, last + 1, logList);
        });

...
public class QSysJwtToken extends RelationalPathBase<SysJwtToken> {
    ...
    QSysJwtToken(String variable) {
        super(SysJwtToken.class, forVariable(variable), "public", "sys_jwt_token");
        addMetadata();
    }

    QSysJwtToken(String variable, String schema, String table) {
        super(SysJwtToken.class, forVariable(variable), schema, table);
        addMetadata();
    }

    QSysJwtToken(String variable, String schema) {
        super(SysJwtToken.class, forVariable(variable), schema, "sys_jwt_token");
        addMetadata();
    }

    QSysJwtToken(Path<? extends SysJwtToken> path) {
        super(path.getType(), path.getMetadata(), "public", "sys_jwt_token");
        addMetadata();
    }

    QSysJwtToken(PathMetadata metadata) {
        super(SysJwtToken.class, metadata, "public", "sys_jwt_token");
        addMetadata();
    }
    ...
    void addMetadata() {
        ...
        addMetadata(token, ColumnMetadata.named("token").withIndex(3).ofType(Types.VARCHAR));
        addMetadata(expiredTime, ColumnMetadata.named("expired_time").withIndex(4).ofType(Types.TIMESTAMP));
        addMetadata(disable, ColumnMetadata.named("disable").withIndex(5).ofType(Types.INTEGER));
        addMetadata(disableReason, ColumnMetadata.named("disable_reason").withIndex(6).ofType(Types.INTEGER));
        addMetadata(refreshToken, ColumnMetadata.named("refresh_token").withIndex(7).ofType(Types.VARCHAR));
        addMetadata(rtExpiredTime, ColumnMetadata.named("rt_expired_time").withIndex(8).ofType(Types.TIMESTAMP));
        addMetadata(rtState, ColumnMetadata.named("rt_state").withIndex(9).ofType(Types.INTEGER));
        addMetadata(rtUseTime, ColumnMetadata.named("rt_use_time").withIndex(10).ofType(Types.TIMESTAMP));
        addMetadata(rtCreateTokenId, ColumnMetadata.named("rt_create_token_id").withIndex(11).ofType(Types.BIGINT));

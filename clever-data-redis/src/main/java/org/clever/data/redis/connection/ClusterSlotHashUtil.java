...
public final class ClusterSlotHashUtil {
    ...
    public static boolean isSameSlotForAllKeys(Collection<ByteBuffer> keys) {
        ...
        return isSameSlotForAllKeys(keys.stream().map(ByteBuffer::duplicate).map(ByteUtils::getBytes).toArray(byte[][]::new));
    }
     ...
     * 从给定的密钥计算插槽
     *
     * @param key 不得为 {@literal null} 或为空
     */
